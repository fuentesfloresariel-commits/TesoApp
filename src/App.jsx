      ...
      <div style={S.main}>
        {(()=>{
          const alDia = alumnos.filter(a=>{
            ...
            // Debe haber pagado al menos la caja chica
            if(!p.has("caja_chica")) return false;
            // No debe tener ninguna cuota vencida sin pagar
            const tieneVencidas = CUOTA_ORDER.some(c=>estaVencida(c,now)&&!p.has(c));
          ...
          <div>
            ...
            {(()=>{
              const pagadoTotal = alumnos.filter(a=>{
                ...
                if(!p.has("caja_chica")) return false;
                return !CUOTA_ORDER.some(c=>estaVencida(c,now)&&!p.has(c));
              }).length;
...
export default function App() {
  ...
  const [pantalla, setPantalla] = useState("codigo");
  const [apoderado, setApoderado] = useState(null);
  const [cargando, setCargando] = useState(true);
  const [error, setError] = useState("");

  // Cargar cursos desde Supabase al iniciar
  useEffect(() => {
    cargarCursos();
  }, []);
  ...
  const cargarCursos = async () => {
    ...
    setError("");
    try {
      const { data, error } = await supabase.from("cursos").select("id, data");
      if (error) throw error;
      const lista = (data || []).map(row => row.data);
      // Si no hay cursos en Supabase, cargamos el demo
      setCursos(lista.length > 0 ? lista : [{...CURSO_DEMO, cobros:[]}]);
    } catch (e) {
      setError("Error conectando con la base de datos. Verifica tu conexión.");
      setCursos([{...CURSO_DEMO, cobros:[]}]);
    } finally {
      setCargando(false);
    }
  };
  ...
  const guardarCurso = async (curso) => {
    try {
      await supabase.from("cursos").upsert({ id: curso.id, data: curso }, { onConflict: "id" });
    } catch (e) {
  ...
  const setCursoActivoData = (updater) => {
    const actualizado = typeof updater === "function" ? updater(cursoActivo) : updater;
    setCursos(cs => cs.map(c => c.id === cursoActivo.id ? actualizado : c));
    setCursoActivo(actualizado);
    guardarCurso(actualizado);
  };

  const salir = () => { setCursoActivo(null); setApoderado(null); setPantalla("codigo"); };

  if (cargando) return (
    <div style={{minHeight:"100vh",background:"#0f1117",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"1rem"}}>
      <div style={{fontFamily:"Georgia,serif",fontSize:"2rem",color:"#e8b84b"}}>TesoAPP</div>
      <div style={{color:"rgba(200,210,230,.6)",fontSize:".9rem"}}>Cargando datos...</div>
    </div>
  );

  if (error) return (
    <div style={{minHeight:"100vh",background:"#0f1117",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"1rem",padding:"2rem"}}>
      <div style={{fontFamily:"Georgia,serif",fontSize:"2rem",color:"#e8b84b"}}>TesoAPP</div>
      <div style={{color:"#f05a6a",fontSize:".9rem",textAlign:"center"}}>{error}</div>
      <button onClick={cargarCursos} style={{background:"#e8b84b",color:"#0f1117",border:"none",borderRadius:8,padding:".6rem 1.4rem",cursor:"pointer",fontWeight:600}}>Reintentar</button>
    </div>
  );
  ...
  if (pantalla === "crear")
    return <PantallaCrearCurso cursos={cursos} onCursoCreado={async c=>{
      const nuevo = {...c, cobros:[]};
      await guardarCurso(nuevo);
      setCursos(cs=>[...cs,nuevo]);
      setCursoActivo(nuevo);
      setPantalla("rol");
    }} onVolver={()=>setPantalla("codigo")}/>;
